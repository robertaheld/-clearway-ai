...
// Tradução dinâmica dos labels com base no idioma selecionado const selectIdioma = document.getElementById("idioma"); selectIdioma.addEventListener("change", () => traduzir(selectIdioma.value));
...
// Upload de imagem (visual) document.getElementById("imagem").addEventListener("change", function (event) { const container = document.getElementById("preview-container"); container.innerHTML = ""; const files = event.target.files; Array.from(files).slice(0, 5).forEach((file) => { const reader = new FileReader(); reader.onload = function (e) { const img = document.createElement("img"); img.src = e.target.result; img.className = "image-preview"; container.appendChild(img); }; reader.readAsDataURL(file); }); });

// Gerar mensagem ao técnico (modo Roberta Held) async function gerarPlano() { const idioma = document.getElementById("idioma").value; const modo = document.getElementById("modo").value; const facial = document.getElementById("facial").value; const classe = document.getElementById("classe").value;

const dentes = []; if (document.getElementById("diastema").checked) dentes.push("diastema"); if (document.getElementById("giroversao").checked) dentes.push("giroversão"); if (document.getElementById("apinhamento").checked) dentes.push("apinhamento"); const detalhesDentes = document.getElementById("detalhesDentes").value;
